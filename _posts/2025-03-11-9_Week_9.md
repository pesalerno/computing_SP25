...
After we re-work the installs of the packages/libraries, if we have time, we will start importing packages and with the afternoon `numpy` activity. 


# Thursday pm | numpy and matplotlib
...
Now, let's talk a bit about how the axes and dimensions work in a `ndarray` object. Here is an array of two axes, the first of length 2 and the second axis has a length of 3: 
...
Let run these lines of code and see what they output: 
...
np.hsplit(a, (3, 4))
```
In numpy, we also have Broadcasting, where you want to carry out an operation between an array and a single number - for example, to convert miles to kilometers:
...
Please submit this by tomorrow (friday March 21st) at the end of the day, using this [drive folder](). 
